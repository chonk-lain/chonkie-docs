...
1. **Process** them with a [Chef](/python/chefs/overview) to parse content
2. **Chunk** them with a [Chunker](/python/chunkers/overview) to split into manageable pieces
3. **Refine** chunks with [Refineries](/python/refinery/overview) for better quality

See the [Pipeline Guide](/python/pipelines) for complete examples.
