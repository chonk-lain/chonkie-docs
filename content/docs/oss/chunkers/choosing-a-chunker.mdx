...
<Cards>
  <Card title="Building a RAG chatbot?" href="/docs/chonkie/oss/chunkers/recursive-chunker">
    Use **RecursiveChunker** for general-purpose document chunking, or **SemanticChunker** if you need high topical coherence.
  </Card>
  <Card title="Processing source code?" href="/docs/chonkie/oss/chunkers/code-chunker">
    Use **CodeChunker** — it understands AST structure and keeps functions/classes intact.
  </Card>
  <Card title="Need maximum throughput?" href="/docs/chonkie/oss/chunkers/fast-chunker">
    Use **FastChunker** — SIMD-accelerated, processes 100+ GB/s at the byte level.
  </Card>
  <Card title="Working with tabular data?" href="/docs/chonkie/oss/chunkers/table-chunker">
...
1. **Are you chunking source code?** → Use [CodeChunker](/docs/chonkie/oss/chunkers/code-chunker)
2. **Are you chunking tables?** → Use [TableChunker](/docs/chonkie/oss/chunkers/table-chunker)
3. **Is raw throughput your top priority?** → Use [FastChunker](/docs/chonkie/oss/chunkers/fast-chunker)
4. **Do you need chunks grouped by topic?**
   - Have an embedding model? → Use [SemanticChunker](/docs/chonkie/oss/chunkers/semantic-chunker)
   - Don't have one? → Use [NeuralChunker](/docs/chonkie/oss/chunkers/neural-chunker)
5. **Are simple sentence-boundary splits enough?** → Use [SentenceChunker](/docs/chonkie/oss/chunkers/sentence-chunker)
6. **None of the above?** → Use [RecursiveChunker](/docs/chonkie/oss/chunkers/recursive-chunker) (best default)
